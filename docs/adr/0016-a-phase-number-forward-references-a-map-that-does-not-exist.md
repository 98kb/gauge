...
**Extended by `docs/adr/README.md`'s citation rule, on one narrow class of in-place edit and on nothing else.**

This ADR fixes the two-regime discipline — *"Before an ADR reaches `main` it is rewritten freely … After it reaches `main` it is only ever appended to"* — and `docs/agents/decision-review.md` enforces it by requiring deletions against `main` to be `0`. **Repointing a rotted file-content URL deletes a line, so the two collide**, and the collision is live rather than theoretical: this repo cites research findings at branch-scoped GitHub URLs, deletes trunk branches on merge, and has already lost one such link — `research/portability-boundary-target-runtimes`, whose disappearance a draft of [ADR 0021](0021-to-brand-identity-composes-with-nothing-at-runtime-and-the-scoping-survives-on-a-different-reason.md) mistook for the findings being unretrievable.

**The carve-out is argued from this ADR's own distinction rather than against it.** The section refusing a repo-wide rename holds that a phase number is *"an **address** and not **reasoning**, so fixing it is closer to **repairing a broken link** than to retracting a claim"*, and it accepts that reply for the ten intact claims — refusing in-place substitution only where the sentence had become **substantively false**, because *"substituting it in place would be a silent repair of a substantive claim, made to look as though it had always been there."* A URL repointed to a commit resolving to the identical bytes repairs no claim, silently or otherwise: it preserves the reasoning a reader saw. The metaphor this ADR reached for is the literal case.

**So: a merged ADR may be edited in place to repoint a file-content URL to a commit-pinned permalink, and for nothing else.** Every deleted line must differ from its replacement in the URL alone, and the PR must list each repointed link with its deletion count, so a non-zero `--numstat` is declared rather than discovered by a reviewer.

**Everything else here is untouched** — the two regimes, `CONTEXT.md`'s amend-in-place treatment, the four footers this ADR lands, its refusal of a repo-wide erasure, and its finding that erasure from the tracker is impossible. **The bright line is unmoved for reasoning of any kind**, which is what this ADR is about; what is added is that an address was never on its side of the line, and the machinery had no way to say so.
