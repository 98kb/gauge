...
> **Two harnesses live under `evals/`, and they answer different questions.**
> This one — `harness/` — drives a scripted founder through a multi-turn
> interview and grades the artifact the session left behind. The other,
> [`inspect-ai/`](inspect-ai/README.md), runs a single agent turn in a Docker
> sandbox with the skill published to it as a tool, and grades the answer, the
> trajectory and the filesystem. `to-vision` and `to-pitch` need the first;
> `gauge` produces no artifact and takes no turns with a persona, so it needs
> the second. Neither replaces the other and they share no code.
